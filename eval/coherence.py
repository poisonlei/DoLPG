...
def cal_graph(data=None, pretrain_model=None):
	...
	for seq in tqdm(data):
		...
		encoding = move2cuda(tokenizer(prompt, next_sentence, return_tensors='pt', add_special_tokens=True, padding=True))
...
def COH(data, pretrain_model):
	...
	for seq in tqdm(data):
		...
		prompt = []
		next_sentence = []
	coh_p = total_prob / (len(data) - cnt)
	coh = total_cnt / (len(data) - cnt)
	print("COH-p: %f\tCOH: %f" % (coh_p, coh))
...
def train(model, tokenizer, criterion, optim, train_data, test_data, batch_size, epoch, checkpoint_path, grad_accum_step):
	model.train()
	best_score = 0
	print()
	for e in range(epoch):
		...
		for i, data in enumerate(data_loader(train_data, batch_size)):
			...
			input_true = tokenizer(prompt, next_sentence, return_tensors='pt', padding=True, add_special_tokens=True)
			output_true = model(**move2cuda(input_true)).sop_logits
			loss_true = criterion(output_true, label_true) / 2
			total_loss += loss_true.item()
			loss_true.backward()

			input_false = tokenizer(next_sentence, prompt, return_tensors='pt', padding=True, add_special_tokens=True)
		...
		with torch.no_grad():
			...
			for data in tqdm(data_loader(test_data, batch_size)):
				...
				input_true = tokenizer(prompt, next_sentence, return_tensors='pt', padding=True, add_special_tokens=True)
				output_true = model(**move2cuda(input_true)).sop_logits
				data_true = F.softmax(output_true, dim=-1)
				total_true += (data_true[:, 0] < 0.5).long().sum().item()

				input_false = tokenizer(next_sentence, prompt, return_tensors='pt', padding=True, add_special_tokens=True)
				output_false = model(**move2cuda(input_false)).sop_logits
				data_false = F.softmax(output_false, dim=-1)
				total_true += (data_false[:, 0] > 0.5).long().sum().item()
...
if __name__ == '__main__':
	...
	if args.train:
		...
		# inference:
		file_save = args.save_file_graph
		file_read = read(args.text_file)

		write(cal_graph(file_read, args.pretrain_model), file_save)
	# COH score
	elif args.coh:
		file_r = read(args.coh_file, split=True)
		COH(file_r, args.pretrain_model)
