...
def data_process(file, word2index, split_sent):
	...
	if split_sent:
		...
		with open(file, 'r', encoding='utf-8') as f:
			data = [line.strip('\n').strip(' ').replace('\t', ' ').lower().split() for line in f.readlines()]
...
def sort_data(data):
	tmp_list = list(enumerate(data))
	tmp_data = sorted(tmp_list, key=lambda x: key_func(x[1]), reverse=True)
	return [(index, value) for index, value in tmp_data]


class data_loader:

	def __init__(self, source, graph, target=None, penalty=None, PAD_index=0, BOS_index=None, EOS_index=None):
		self.source = source
		self.graph = graph
	...
	def _get_tokens(self, shuffle, args):
		...
		for (st, ed) in tqdm(index_pair):
			...
			if self.train_flag:
				...
				data.append((
					LongTensor(pad_batch(self.source[st:ed], self.PAD_index, dim=3)), graph,
					LongTensor(pad_batch(self.target_input[st:ed], self.PAD_index, dim=2)), target_output, penalty))
			else:
...
def preprocess():
	...
	source = data_process(file=args.source, word2index=word2index, split_sent=True)
	target = None
	src_line = None
	penalty = None
	if args.target is not None:
		print("===> Create target dataset.")
		target = data_process(file=args.target, word2index=word2index, split_sent=False)
		print("===> Create penalty coefficient.")
		src_line = data_process(file=args.source, word2index=word2index, split_sent=False)
		penalty = cal_penalty(src_line, target)
	print("===> Create graph dataset.")
	graph = graph_process(file=args.graph)
	...
	dataloader = data_loader(
		source=source, penalty=penalty, target=target, graph=graph, PAD_index=constants.PAD_index, BOS_index=constants.BOS_index,
