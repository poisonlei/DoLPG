...
class Fit:
	...
	def train_ce(self, source, target_inputs, target_outputs, norm):
		outputs = self.model(source, mode='train', target=target_inputs, return_memory=False, step='ce')
	...
	def run(self, data):
		...
		for i, (source, graph, target_inputs, target_outputs, penalty) in enumerate(data):
			...
			total_tok += ntoken
			total_cnt += 1
			outputs = self.model(mode='train', source=source, graph=graph, ground_truth=target_inputs)
	...
	def __call__(self, start_epoch):
		...
		for epoch in range(start_epoch, self.EPOCH):
			...
			if self.rank == 0 and self.checkpoint is not None:
				self.checkpoint.save_point(model=self.model, optim=self.optimizer, epoch=epoch + 1)
