...
def trainer(gpu, args, seed):
	...
	criterion = LabelSmoothing(smoothing=args.smoothing, ignore_index=constants.PAD_index).cuda(args.rank)
	check_point = None
	check_point = checkpoint(save_path=args.checkpoint_path, checkpoint_num=args.checkpoint_num, restore_file=args.restore_file, rank=gpu)
	params = {}
	...
	optimizer = WarmUpOpt(
		optimizer=optim.Adam(
			params=model.parameters(), lr=args.learning_rate, betas=(args.beta_1, args.beta_2), eps=args.eps,
			weight_decay=args.weight_decay),
		d_model=args.embedding_dim, warmup_steps=args.warmup_steps, min_learning_rate=args.min_learning_rate, factor=args.factor,
	...
	train_dataset, batch_size = train_data.set_param(shuffle=True, args=args, train_flag=True, seed=seed)
	torch.cuda.set_device(gpu)
	train_sampler = distributed.DistributedSampler(train_dataset, num_replicas=args.world_size, rank=args.rank)
