...
class Decoder(nn.Module):
	...
	def generate(self, embed, encoder_outputs, src_pad_mask, memory=None):
		...
		for (i, decoder) in enumerate(self.decoder):
			embed, memory_new, weight = decoder.memory_decode(
				inputs=embed, memory=memory, memory_new=memory_new, encoder_outputs=encoder_outputs, pad_mask=src_pad_mask, i=i)
		if self.normalize_before:
...
class LabelSmoothing(nn.Module):
	...
	def forward(self, inputs, targets, penalty=None):
		...
		loss = self.criterion(inputs.view(-1, vocab_size), targets.view(-1, vocab_size).detach()).sum(dim=-1)
...
class WarmUpOpt:
	...
	def __init__(self, optimizer, d_model, warmup_steps, min_learning_rate, factor=1, state_dict=None):
		self.step_num = 0
		self.factor = factor
		self.d_model = d_model
		...
		if state_dict is not None:
			self.load_state_dic(state_dict)
	...
	def load_state_dic(self, state_dict):
