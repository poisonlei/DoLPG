...
from .Module import (Encoder, Decoder, Embedding, PositionWiseFeedForwardNetworks, MultiHeadAttention, EncoderCell, DecoderCell, pad_mask,
					 triu_mask, SublayerConnection, Linear, clones)
...
class transformer(nn.Module):
	def __init__(self, config):
		...
		num_layer_graph_encoder = getattr(config, 'num_layer_graph_encoder', 6)
		...
		self.src_embed = Embedding(vocab_size, d_model, dropout=dropout_embed, padding_idx=PAD_index)
		self.tgt_embed = self.src_embed

		normalize_before = getattr(config, 'normalize_before', False)

		self.sent_Encoder = Encoder(
			d_model=d_model, num_layer=num_layer_encoder, layer=EncoderCell(
				d_model=d_model, attn=attention, FFNlayer=FFN, dropout=dropout_sublayer, normalize_before=normalize_before),
			normalize_before=normalize_before)
		self.graph_Encoder_f = GAT(d_model=d_model, num_layer=num_layer_graph_encoder, num_head=num_head, dropout=dropout_sublayer)

		self.graph_Encoder_r = GAT(d_model=d_model, num_layer=num_layer_graph_encoder, num_head=num_head, dropout=dropout_sublayer)
		...
		self.Decoder = Decoder(
			d_model=d_model, layer=DecoderCell(
				d_model=d_model, attn=attention, FFNlayer=FFN, dropout=dropout_sublayer, normalize_before=normalize_before),
			num_layer=num_layer_decoder, normalize_before=normalize_before)

		beam = getattr(config, 'beam', 5)
		search_method = getattr(config, 'decode_method', 'greedy')
		self.decode_search = SearchMethod(search_method=search_method, BOS_index=BOS_index, EOS_index=EOS_index, beam=beam)
	...
	def generate(self, embed, encoder_outputs, src_pad_mask, source, memory=None):
		outputs, memory, attn_weight = self.Decoder.generate(
			embed=embed, encoder_outputs=encoder_outputs, src_pad_mask=src_pad_mask, memory=memory)
	...
	def forward(self, **kwargs):
		...
		if kwargs['mode'] == 'train':
			...
			outputs, attn_weight = self.Decoder(inputs=tgt_embed, encoder_outputs=encoder_outputs, pad_mask=encoder_mask, seq_mask=tgt_mask)
			p_g = torch.sigmoid(self.w_p_g(outputs))
			outputs = self.project(outputs)
			prob = F.softmax(outputs, dim=-1)
			...
			return self.decode_search(
				decoder=self.generate, tgt_embed=self.tgt_embed.single_embed, src_pad_mask=encoder_mask, encoder_outputs=encoder_outputs,
				max_length=max_length, source=source)
